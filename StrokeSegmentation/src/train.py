...
def main():
    ...
    train_dataset = SegmentationDataset(
        val=False, transform=data_transforms["train"], image_size=config["image_size"]
    )

    val_dataset = SegmentationDataset(
        val=True, transform=data_transforms["val"], image_size=config["image_size"]
    )
    ...
    # 保存模型
    torch.save(trained_model.state_dict(), settings.SAVE_PATH)
    print("Model saved as " + settings.SAVE_PATH)


if __name__ == "__main__":
    print("\nCurrent working path: " + os.getcwd())
    print(settings)
    print("Are you ready to start training? (y/n)")
    if input() == "y":
        main()
    else:

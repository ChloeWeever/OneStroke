...
class Evaluator(nn.Module):
    ...
    def _calculate_accuracy_per_class(self, prediction, target, weights, class_idx):
        ...
        for true_label, true_comp_mask_uint8 in true_components:
            ...
            # 找到与当前真实连通域重叠最多的预测连通域
            best_overlap = 0
            best_pred_comp_mask = None

            for pred_label, pred_comp_mask_uint8 in pred_components:
                pred_comp_mask = pred_comp_mask_uint8.astype(bool)

                # 计算重叠比例（交集/真实面积）
                overlap = np.logical_and(true_comp_mask, pred_comp_mask).sum()
                overlap_ratio = overlap / (area_true + 1e-10)

                if overlap_ratio > best_overlap:
                    best_overlap = overlap_ratio
                    best_pred_comp_mask = pred_comp_mask

            # 如果没有找到匹配的预测连通域，使用整个预测图像作为fallback
            if best_pred_comp_mask is None:
                best_pred_comp_mask = pred_k

            # 计算该连通域内的加权准确率（基础值）
            correct_pixels = np.logical_and(true_comp_mask, pred_k)
            weights_comp = weights_np * true_comp_mask
            ...
            if base_accuracy >= self.validity_threshold:
                comp_score = 100.0 * base_accuracy
            else:
                # 无效连通域：统计丢失落入其它类别的种类数（wrong_section）
                wrong_section = 0
                for other_pred in other_pred_np_list:
                    if np.logical_and(true_comp_mask, other_pred).sum() > 0:
                        wrong_section += 1

                # 2. 计算 P_i_k
                P_i_k = wrong_section / (4.0 * self.validity_threshold)
    ...
    def _calculate_accuracy(self, mask_path, prediction):
        """
        计算整字预测准确率
        ...
        """
        # 计算权重
        targets = self.load_masks_from_dir(mask_path)
    ...
    def main(
        ...
        # 2. 计算准确率
        accuracy_result = self._calculate_accuracy(mask_path, pred_tensor)

        return accuracy_result


if __name__ == '__main__':
    ture_mask_path = Path("data/output_img/16/0")  # 真实图像的二值化掩码
    weight_path = Path("models/unet_model_4.pth")  # 更改模型
    result_path = Path("data/output_img/42/16/0.jpg")  # 需要预测图像

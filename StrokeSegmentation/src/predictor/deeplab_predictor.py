...
class deeplabv3Predictor:
    def __init__(self, model_path, device='cuda'):
        ...
        self.model = DeepLabV3Plus(n_channels=3, n_classes=6, pretrained_backbone=False, use_se=True).to(self.device)
        try:
            self.model.load_state_dict(torch.load(model_path, map_location=self.device))
            self.model.eval()
        except Exception as e:
            print(f"加载DeepLabV3+模型时出错: {e}")
            print("请确保模型文件 'models/deeplab_model_new.pth' 存在")

    ...
    def _calculate_accuracy_per_class(self, prediction, target, weights, class_idx):
        ...
        for _, comp_mask_uint8 in true_components:
            ...
            if overlap_ratio >= self.validity_threshold:
                ...
                comp_score = 100.0 * (weighted_correct / (weighted_total + 1e-10))
            else:
                ...
                # 分数限制在 0~60 范围：60 * (overlap_ratio * penalty)
                comp_score = 60.0 * (overlap_ratio * penalty)
...
if __name__ == '__main__':
    target_path = Path("../data/output_img/39/0")  # 真实图像的二值化掩码
    weight_path = Path("../models/unet_model_3.pth")  # 更改模型
    result = Path("../data/output_img/39/0/0.jpg")  # 需要预测图像
    evaluator = Evaluator(validity_threshold=0.6)

    # 兼容本项目unet模型代码
    accuracy_result = evaluator.main(target_path, weight_path, result, threshold=0.5)

    # 其余模型调用请仿照此处，添加模型实例化代码,其中模型预测返回结果result形状为 (H, W, 1)
    # model = UNetPredictor(weight_path)
    # accuracy_result = evaluator.main(target_path, None, result, threshold=0.5, predict_fn=model.predict)

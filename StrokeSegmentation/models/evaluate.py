...
class Evaluator(nn.Module):
    ...
    def _calculate_accuracy_per_class(self, prediction, target, weights, class_idx):
        ...
        if iou >= 0.6:
            ...
            # 计算优化后的分数 (限制在0-60范围内)
            pred_area = pred_k.sum().item()
            true_area = target_k.sum().item()
            ratio = pred_area / (true_area + 1e-10)
            print(f"第{class_idx + 1}占比:{ratio}" f"损失率:{wrong_section}")
            score = 60 * (ratio ** wrong_section)
...
if __name__ == '__main__':
    target_path = f"D:\OneStroke-main\OneStroke-main\StrokeSegmentation\data\output_img\\11\\0"  #真实图像的二值化掩码
